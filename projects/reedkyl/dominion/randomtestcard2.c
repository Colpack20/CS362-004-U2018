...
int main() {
	...
	printf("Add council_room to index 0 (hand position 1) and use card effect of council_room on player 1\n");
	int b;
	for(b =0; b < 500; b++){
	seed = rand();
				...
				cardEffect(council_room, choice1, choice2, choice3, &testcaseG, handpos, bonus);//play smithy from index 0 of hand
				...
				if(testcaseG.numBuys == G.numBuys + 1)
					passedTests++;
				//printf("Passed: there is 1 more buy for player 1\n");
				else{
					printf("Failed: there isn't 1 more buy for player 1, there are %d buys and there were expected to be %d buys\n", testcaseG.numBuys, G.numBuys + 1);
					failedTests++;
				}
				...
				//printf("Passed: 1 card was drawn from Player 1's deck\n");
					...
					printf("Failed: %d card(s) were drawn from Player 1's deck, and 4 cards were expected to be drawn\n", G.deckCount[currentPlayer] - testcaseG.deckCount[currentPlayer]);
					failedTests++;
				}
				if(testcaseG.handCount[1] == G.handCount[1] + 1)
					passedTests++;
				//printf("Passed: Player 2's hand count didn't change\n");
				else{
					printf("Failed: Player 2's hand count didn't increase by 1.  The actual hand count is %d and the expected hand count was %d\n", testcaseG.handCount[1], G.handCount[1] + 1);
					failedTests++;
				}
				if(testcaseG.deckCount[1] == G.deckCount[1] - 1)
					passedTests++;
				//printf("Passed: Player 2's deck count didn't change\n");
				else{
					printf("Failed: Player 2's deck count didn't decrease by 1. The actual deck count is %d and the expected hand count was %d\n", testcaseG.deckCount[1], G.deckCount[1] - 1);
					failedTests++;
				}
